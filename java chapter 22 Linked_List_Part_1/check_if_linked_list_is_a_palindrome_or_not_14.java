public class check_if_linked_list_is_a_palindrome_or_not_14 {
 ...
 if(head.data==key){ // if head.data = = key 
   ...
   public void deleteNthFromEnd(int n){
    ...
    int size = 0;
    Node temp = head; // initially temp is pointing to the head of the linked list
    while(temp!=null){
        temp = temp.next;
        size++;
    }
    // if n = size of the linked list then we have to remove the first node of the linked list 
    // always remember nth node from the end == (size-n+1)th node from the start of the linked list  .........................
    if (n==size){ // remove first 
        head = head.next; // removing the first node of the linked list 
        return;
    }
    // if n > size of the linked list then we cannot remove any node from the linked list as it is invalid 
    if (n>size){
        System.out.println("Invalid n value");
        return;
    }
    // if n < size of the linked list then we have to remove the (size - n)th node from the start of the linked list 
    // size-n 
    int  i = 1;
    int iToFind = size - n; // index of the node to be removed from the start of the linked list 
    Node prev = head; // previous node 
    while(i<iToFind){ // previous will run till size-n-1 so maximum value of prev.next will be equal to size-n only
        prev = prev.next;
        i++;
    }
    // breaking of the linkage . ......
    prev.next = prev.next.next; // removing the nth node from the end of the linked list by changing the next pointer of the previous node to point to the next node of the node to be removed 
   }
   public Node findMid (Node head){
    Node slow = head ; 
    Node fast = head; 
    // in case of odd no of nodes --> condition is fast.next!= null
    // in case of even no of nodes --> condition is fast!= null
   ...
   public boolean checkPalindrome(){
    ...
    if (head == null || head.next == null){
        // if linked list is empty or linked list has only one element then linked list is automatically considered as palindrome .....
        return true; // empty linked list or linked list with only one node is always a palindrome 
    }
   ...
   Node prev = null ;
   Node curr = midNode; // as we reverse the 2nd half of the linked list so curr is pointing to the mid node
   // starting badal jaiyegi baaki saari steps same rahengi ....
    Node next ;
    while (curr!=null){
        next = curr.next; // step 1 : defining the next node 
        curr.next = prev; // step 2 
        prev = curr; // step 3
        curr = next; // step 4
    }
   // step 3 : check whether 1st half and 2nd half are identical or not
    Node right = prev; // right half head as previous is my last node and after reversing the linked list it will become the head of the 2nd half of the linkeed list .......
    Node left = head; // left half head
    while (right!= null){
        if (left.data != right.data){
            return false; // not a palindrome 
        }
        left = left.next; // updating 
        right = right.next;  // updating 
    }
    return true;
}
    public static void main (String args[]){
check_if_linked_list_is_a_palindrome_or_not_14 aman = new  check_if_linked_list_is_a_palindrome_or_not_14();
   aman.printLL();
    ...
    // aman.reverse();
    // aman.printLL();
    // aman.deleteNthFromEnd(3); // 1200 will be removed from the linked list
    // aman.printLL();
...
check_if_linked_list_is_a_palindrome_or_not_14 arun = new  check_if_linked_list_is_a_palindrome_or_not_14();
arun.addLast(1);
arun.addLast(2);
arun.addLast(2);
// arun.addLast(1); // 1->2->2->1
arun.printLL();
System.out  .println(arun.checkPalindrome()); // true
arun.addLast(3); // 1->2->2->1->3
arun.printLL();
System.out.println(arun.checkPalindrome()); // false
